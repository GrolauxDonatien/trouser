            ...
            switch (prop) {
                case "listener":
                    for (let l in what.listener) {
                        let events = what.listener[l];
                        if (Array.isArray(events)) {
                            for (let i = 0; i < events.length; i++) {
                                el.addEventListener(l, events[i]);
                            }
                        } else {
                            el.addEventListener(l, events);
                        }
                    }
                    break;
                case "child":
                ...
                case "mapChildren":
                    ...
                    lel.mapChildren(isLive(children) ? children : live(children), c => build.innerBuild(what[prop].render(c), context));
                    break;
                case "innerText":
                case "innerHTML":
                case "value":
